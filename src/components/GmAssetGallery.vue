<template>
  ...
  <div class="asset-grid grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-4 flex-1 overflow-y-auto min-h-0 pb-2 w-full grid-auto-rows-[1fr] max-h-72 scrollbar-thin scrollbar-thumb-stone-700 scrollbar-track-stone-900">
        <div 
          ...
          <div class="thumbnail-overlay absolute inset-0 bg-black/70 flex items-center justify-center opacity-0 transition-all z-10 hover:opacity-100">
            <div class="overlay-actions flex flex-col gap-2 items-center">
              ...
              <button 
                ...
                class="push-btn bg-linear-to-br from-blue-500 to-blue-700 text-white rounded-lg px-4 py-2 text-sm font-semibold shadow hover:-translate-y-0.5 transition disabled:opacity-60 disabled:cursor-not-allowed"
              >
                {{ pushing === asset.id ? '⏳' : '📺' }}
...
onMounted(async () => {
  ...
  unsubscribeRef.value = assetStore.subscribeToChanges()
})
